...
\item Sample variance of the response, $\Var(\vy) := \vy^T\vy/(n-1)$ (or, alternatively, $\vy^T\vy/n$). Note here $\vy$ is pre-centered. 
...
Using $\mx^T\mx$, $\mx^T\vy$, $n$ and $\vy^T\vy$ as input, results for (2.7) -- (2.11), (2.18) in the manuscript still hold. For (A.29) -- (A.36) the only missing part is $\mx^T\er$, which can be written as:
\begin{align}
...
where
\begin{equation}
    \hat{\sigma}_{j}^{2} = \frac{\norm{\vy - \hat{b}_0 - \vx_{j} \hat{b}_{j}}^2}{n-2} = \frac{RSS_{j}}{n-2}
\end{equation}
...
Correlation coefficient $R^2$ for the corresponding simple linear regression model is
\begin{equation}
...
\begin{equation}
    1-R_{j}^{2} = \frac{RSS_j}{SSreg_j + RSS_j} = \frac{\hat{\sigma}_{j}^2(n-2)}{\vy^{T}\vy}
\end{equation}
\begin{equation}
